...
*（第 4 课试一试 Q1 判对；第 5 课回顾以 Cucumber + mock = S2 复验通过，软点结案。）*

### S2（孤立六边形）
开发顺序矩阵中：驱动侧=测试、六边形=真实、被驱动侧=替身。**此阶段达成六边形架构主目标**
——业务可在无 UI、无真库下经驱动端口回归。S5（全真）是生产部署，不是「目标完成」的标记。
*（第 4 课 lesson4_isolation.cc 形态已练；第 5 课回顾复验 S2≠S5 通过，见 learning-records/0006。）*

### 依赖规则 Dependency Rule
源码依赖只能从外侧机制指向内侧政策；内侧代码不得提到外侧声明的类名，外部 DTO / 数据格式
跨边界时应转换。评审不看目录自报身份，而看编译依赖；再用 S2 孤立回归补动态证据。
Hexagonal、Clean、Onion 都兼容这条共同内核，但观察轴不同：Hexagonal 看参与者对话，Clean 看
政策/机制圈层，Onion 看领域模型居中。传统分层不是天然错误，应按领域复杂度与变化压力决定
是否值得反转依赖。
*（第 5 课练一练 8/8、试一试 5/5，2026-07-20；见 learning-records/0006。）*
